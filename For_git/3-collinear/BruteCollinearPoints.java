...
public class BruteCollinearPoints {
    ...
    public BruteCollinearPoints(Point[] points) { // this is the constructor
        if (points == null) {
        ...
        /* 1) Loop through all the pairs of points
           ...
           7) Loop through the collinearPointsEnd array, if the ends of the LS are alr inside
            ...
            b) else add in the ends (check the array capacity, if not enough, double the size, increase mutableSegments count
        ...
        for (int i = 0; i < points.length; i++) {
            for (int j = i + 1; j < points.length; j++) {
                ...
                if (countPair >= 4) {
                    ...
                    if (flag2 == 1) {
                        if (collinearPointsEnd.length <= mutableSegments) {
                            collinearPointsEnd = resize(collinearPointsEnd, 2 * mutableSegments);
                        }
                        collinearPointsEnd[mutableSegments] = lineSegmentEnds;
                        mutableSegmentsResult[mutableSegments] = new LineSegment(max, min);
                        mutableSegments += 1;
                    }
                }
            }
